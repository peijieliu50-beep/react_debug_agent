...
from web.deploy_guard import (
    ...
    get_daily_limit, get_access_password, resolve_low_memory,
)
...
# 网页端默认开启低内存模式：训练验证走静态分析降级，不再启动第二个 torch 子进程，
# 避免“跑到训练那轮卡死 / 内存溢出导致 Streamlit 闪退”。可在 Secrets 设 LOW_MEMORY="0" 关闭。
LOW_MEM = resolve_low_memory(default="1")
...
with left:
    ...
    # 低内存模式提示：网页端训练验证走静态分析降级，避免内存溢出闪退
    if LOW_MEM:
        st.caption("🛡️ 低内存模式：训练验证走静态分析（不在网页端实跑），本地命令行可完整运行训练。")
